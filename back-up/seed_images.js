...
async function forceSeed() {
    ...
    try {
        ...
        for (const [id, url] of Object.entries(menuImages)) {
            const [result] = await pool.query('UPDATE MENU SET IMAGE_URL = ? WHERE MENU_ID = ? AND (IMAGE_URL IS NULL OR IMAGE_URL = "")', [url, id]);
            updatedCount += result.affectedRows;
        }

        console.log(`Updated ${updatedCount} system menu items with images.`);
        console.log('Done.');
