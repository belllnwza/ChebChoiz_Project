...
async function hardRestore() {
    ...
    try {
        console.log('--- HARD RESTORE START ---');

        // 1. DELETE ALL SYSTEM MENUS (ID < 10000 is safer definition of system menu than NULL user if NULL is corrupted)
        await pool.query('DELETE FROM MENU WHERE MENU_ID < 20000');
        console.log('Deleted all legacy System Menus.');
        ...
        console.log(`Re-inserted ${count} System Menus.`);

        // 3. FORCE CLEAR HIDDEN (GLOBAL RESET)
        await pool.query('DELETE FROM USER_HIDDEN_MENU');
        console.log('Cleared all HIDDEN flags.');
