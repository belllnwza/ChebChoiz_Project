...
function deriveMenuName(url) {
    ...
    const specialNames = {
        "shabu": "Shabu Shabu", "mookrata": "Mookrata", "padtai": "Pad Thai", "steamedeggbuns": "Steamed Egg Buns",
        "shrimpfriedrice": "Shrimp Fried Rice", "porkporridge": "Pork Porridge", "padseeew": "Pad See Ew",
        "mangostickyrice": "Mango Sticky Rice", "kimchisoup": "Kimchi Soup", "honeytoast": "Honey Toast",
        "frenchfries": "French Fries", "curryrice": "Curry Rice", "chickenbasil": "Chicken Basil",
        "boatnoodles": "Boat Noodles", "beefbowl": "Beef Bowl", "macandcheese": "Mac & Cheese",
        "corncream": "Corn Cream", "tomyum": "Tom Yum", "pineapplefriedrice": "Pineapple Fried Rice",
        "hainanesechickenrice": "Hainanese Chicken Rice", "hokkeinmee": "Hokkien Mee", "kuayjub": "Kuay Jub",
        "sichuansoup": "Sichuan Soup", "chocolatebuffet": "Chocolate Buffet", "fishmawsoup": "Fish Maw Soup",
        "beefwellington": "Beef Wellington", "bostonlobster": "Boston Lobster",
        "freshberrybuffet": "Fresh Berry Buffet", "fettuccinealfredo": "Fettuccine Alfredo",
        "herbroastedchicken": "Herb Roasted Chicken", "foiegras": "Foie Gras", "fishchips": "Fish & Chips",
        "mapotofu": "Mapo Tofu", "garlicbread": "Garlic Bread", "sashimi": "Sashimi Buffet",
        "chickenwrap": "Chicken Wrap", "roastducknoodles": "Roast Duck Noodles", "chickenwings": "Chicken Wings",
        "americanfriedrice": "American Fried Rice", "abalonesoup": "Abalone Soup", "porkbonesoup": "Pork Bone Soup",
        "mushroom_soup": "Mushroom Soup", "grillandbbq": "Grill & BBQ", "tomatosoup": "Tomato Soup", "yentafo": "Yen Ta Fo"
    };
...
async function run() {
    ...
    try {
        console.log('--- EMERGENCY MANUAL RESTORE ---');

        // 1. DELETE SYSTEM MENUS (ID < 10000)
        // I use 20000 to be safe
        await pool.query('DELETE FROM MENU WHERE MENU_ID < 20000');
        console.log('Deleted system menus.');

        // 2. INSERT 80 ITEMS
        let count = 0;
        for (const [id, url] of Object.entries(menuImages)) {
            const name = deriveMenuName(url);
            const sql = `INSERT INTO MENU (MENU_ID, MENU_NAME, PRICE_ID, IMAGE_URL, MENU_PRICE, USER_ID) VALUES (?, ?, 10003, ?, 0, NULL)`;
            await pool.query(sql, [id, name, url]);
            count++;
        }
        ...
        console.log('Cleared hidden menus.');
