...
function deriveMenuName(url) 
{
    if (!url) return "Unknown Menu";
    ...
    const specialNames =
    {
        ...
        "mookrata"             : "Mookrata",
        "padtai"               : "Pad Thai",
        "steamedeggbuns"       : "Steamed Egg Buns",
        "shrimpfriedrice"      : "Shrimp Fried Rice",
        "porkporridge"         : "Pork Porridge",
        "padseeew"             : "Pad See Ew",
        "mangostickyrice"      : "Mango Sticky Rice",
        "kimchisoup"           : "Kimchi Soup",
        "honeytoast"           : "Honey Toast",
        "frenchfries"          : "French Fries",
        "curryrice"            : "Curry Rice",
        "chickenbasil"         : "Chicken Basil",
        "boatnoodles"          : "Boat Noodles",
        "beefbowl"             : "Beef Bowl",
        "macandcheese"         : "Mac & Cheese",
        "corncream"            : "Corn Cream",
        "tomyum"               : "Tom Yum",
        "pineapplefriedrice"   : "Pineapple Fried Rice",
        "hainanesechickenrice" : "Hainanese Chicken Rice",
        "hokkeinmee"           : "Hokkien Mee",
        "kuayjub"              : "Kuay Jub",
        "sichuansoup"          : "Sichuan Soup",
        "chocolatebuffet"      : "Chocolate Buffet",
        "fishmawsoup"          : "Fish Maw Soup",
        "beefwellington"       : "Beef Wellington",
        "bostonlobster"        : "Boston Lobster",
        "freshberrybuffet"     : "Fresh Berry Buffet",
        "fettuccinealfredo"    : "Fettuccine Alfredo",
        "herbroastedchicken"   : "Herb Roasted Chicken",
        "foiegras"             : "Foie Gras",
        "fishchips"            : "Fish & Chips",
        "mapotofu"             : "Mapo Tofu",
        "garlicbread"          : "Garlic Bread",
        "sashimi"              : "Sashimi Buffet",
        "chickenwrap"          : "Chicken Wrap",
        "roastducknoodles"     : "Roast Duck Noodles",
        "chickenwings"         : "Chicken Wings",
        "americanfriedrice"    : "American Fried Rice",
        "abalonesoup"          : "Abalone Soup",
        "porkbonesoup"         : "Pork Bone Soup",
        "mushroom_soup"        : "Mushroom Soup",
        "grillandbbq"          : "Grill & BBQ",
    ...
    try 
    {
        ...
        for (const [table, cols] of Object.entries(columns)) 
        {
            for (const col of cols) 
            {
                try 
                {
                    await connection.query(`SELECT ${col} FROM ${table} LIMIT 1`);
                }
                catch 
                {
                    console.log(`Adding ${col} to ${table}...`);
                    const type = col === 'USER_ID' ? 'INT DEFAULT NULL' : 'VARCHAR(255)';
                    const defaultVal = col === 'PASSWORD' ? "DEFAULT '1234'" : "";
                    await connection.query(`ALTER TABLE ${table} ADD COLUMN ${col} ${type} ${defaultVal}`);
                }
            }
        }
        ...
        catch (e) { console.log("ID Modify: " + e.message); }
        ...
        {
            if (e.code !== 'ER_DUP_FIELDNAME') console.log("IS_DELETED Add: " + e.message);
        }
        ...
        await ensureSystemMenus(connection);

        connection.release();
        console.log('Database Schema & Data verified.');
    }
    catch (error) 
    {
...
async function ensureSystemMenus(conn) 
{
    console.log('Checking System Menus integrity...');

    for (const [id, url] of Object.entries(menuImages)) {
        const name = deriveMenuName(url);
        const priceId = 10003;
        const sql = `
            INSERT INTO MENU (MENU_ID, MENU_NAME, PRICE_ID, IMAGE_URL, MENU_PRICE, USER_ID)
            VALUES (?, ?, ?, ?, 0, NULL)
            ...
            USER_ID = NULL
        `;
        await conn.query(sql, [id, name, priceId, url]);
    }

    const currentIds = Object.keys(menuImages).map(Number);
    if (currentIds.length > 0) 
    {
        try 
        {
            const placeholders = currentIds.map(() => '?').join(',');
            await conn.query(`DELETE FROM MENU_CATEGORY WHERE MENU_ID < 10000 AND MENU_ID NOT IN (${placeholders})`, currentIds);
            await conn.query(`DELETE FROM MENU_SITUATION WHERE MENU_ID < 10000 AND MENU_ID NOT IN (${placeholders})`, currentIds);
            await conn.query(`DELETE FROM MENU WHERE MENU_ID < 10000 AND MENU_ID NOT IN (${placeholders})`, currentIds);
        }
        catch (err) 
        {
            console.log('[SYNC] Note: Some system menus were not deleted because they are referenced by history.');
        }
    }
    console.log('System Menus Synced & Cleaned (Categories preserved in DB).');
}
...
app.post('/api/login', async (req, res) => 
{
    ...
    try 
    {
        const [users] = await pool.execute('SELECT * FROM USER WHERE BINARY USER_NAME = ? AND BINARY PASSWORD = ?',
            [username, password]);
...
app.post('/api/reset-password', async (req, res) => 
{
    ...
    try 
    {
        const [result] = await pool.execute('UPDATE USER SET PASSWORD = ? WHERE BINARY USER_NAME = ?',
    ...
    try 
{
        ...
        for (const r of rows) 
        {
            if (r.category === null) 
            {
                orphans.push({ MENU_ID: r.MENU_ID, name: r.name });
            }
        }

        if (orphans.length > 0) 
        {
            await repairOrphanedHistory(pool, userId, orphans);
            const [rowsRefreshed] = await pool.execute(sql, [userId]);
            rows.splice(0, rows.length, ...rowsRefreshed);
        }
...
app.post('/api/menu/add', async (req, res) => 
    ...
    try 
    {
        ...
        while (newId < 10000) 
        {
            const [check] = await pool.execute('SELECT MENU_ID FROM MENU WHERE MENU_ID = ?', [newId]);
        ...
        if (newId >= 10000) 
        {
            return res.status(500).json({ success: false, message: 'ID range full (2000-9999)' });
        }

        const priceId = mapPriceLevel(priceLevel);
        const finalImage = (imageUrl && imageUrl.trim() !== '' && !imageUrl.toLowerCase().includes('default'))
            ? imageUrl
            : 'img/logo.png';
...
// 9. DELETE MENU (Soft Delete for Custom, Hide for System)
app.post('/api/menu/delete', async (req, res) => 
{
    ...
    try 
    {
        ...
        if (isCustom)
        {
            ...
            await pool.execute('UPDATE MENU SET IS_DELETED = 1 WHERE MENU_ID = ?', [menuId]);
        ...
        res.json({ success: true, message: 'Menu hidden/deleted' });
    }
    catch (err) 
    {
        console.error(err);
...
app.post('/api/menu/edit', async (req, res) => 
{
    ...
    try 
    {
        ...
        const finalImage = (imageUrl && imageUrl.trim() !== '' && !imageUrl.toLowerCase().includes('default'))
            ? imageUrl
            : (existingMenu.IMAGE_URL || 'img/Logo.png');
        ...
        else 
        {
            ...
            const finalImageForCopy = (imageUrl && imageUrl.trim() !== '' && !imageUrl.toLowerCase().includes('default'))
                ? imageUrl
                : (existingMenu.IMAGE_URL || 'img/Logo.png');

            await pool.execute(
                'INSERT INTO MENU (MENU_ID, MENU_NAME, PRICE_ID, IMAGE_URL, USER_ID, MENU_PRICE) VALUES (?, ?, ?, ?, ?, 0)',
                [newId, name, priceId, finalImageForCopy, userId]
            );
            finalMenuId = newId;
        }

        const [sourceCats] = await pool.execute('SELECT CATE_ID FROM MENU_CATEGORY WHERE MENU_ID = ?', [menuId]);
        const [sourceSits] = await pool.execute('SELECT SIT_ID FROM MENU_SITUATION WHERE MENU_ID = ?', [menuId]);
        const existingCatIds = sourceCats.map(c => c.CATE_ID);
        const existingSitIds = sourceSits.map(s => s.SIT_ID);

        let finalCategoryIds = (categoryIds === undefined) ? existingCatIds : categoryIds;
        let finalSituationIds = (situationIds === undefined) ? existingSitIds : situationIds;

        if (!Array.isArray(finalCategoryIds)) finalCategoryIds = [];
        if (!Array.isArray(finalSituationIds)) finalSituationIds = [];
        ...
        if (finalCategoryIds.length > 0) {
            for (const cateId of finalCategoryIds) 
            {
                await pool.execute('INSERT INTO MENU_CATEGORY (MENU_ID, CATE_ID) VALUES (?, ?)', [finalMenuId, cateId]);
            }
        }

        if (finalSituationIds.length > 0) {
            for (const sitId of finalSituationIds) 
            {
...
// 11. RESET MENU (Soft Reset)
app.post('/api/menu/reset', async (req, res) => 
{
    const { userId } = req.body;
    try 
    {
        await pool.execute('UPDATE MENU SET IS_DELETED = 1 WHERE USER_ID = ?', [userId]);
...
async function repairOrphanedHistory(pool, userId, orphans) 
{
    if (orphans.length === 0) return;
    console.log(`[HISTORY REPAIR] Detected ${orphans.length} orphans for user ${userId}. Attempting repair...`);
    ...
    try {
        for (const orphan of orphans) 
        {
            const [donors] = await conn.query(`
                ...
                AND m.MENU_ID != ?
                LIMIT 1
            `, [orphan.name, orphan.MENU_ID]);

            if (donors.length > 0) 
            {
                const donorId = donors[0].MENU_ID;
                const [cats] = await conn.query("SELECT CATE_ID FROM MENU_CATEGORY WHERE MENU_ID = ?",
                    [donorId]);

                for (const c of cats) 
                {
                    await conn.query("INSERT IGNORE INTO MENU_CATEGORY (MENU_ID, CATE_ID) VALUES (?, ?)",
                        [orphan.MENU_ID, c.CATE_ID]);
                }
                ...
                for (const s of sits) 
                {                
                    await conn.query("INSERT IGNORE INTO MENU_SITUATION (MENU_ID, SIT_ID) VALUES (?, ?)",
                        [orphan.MENU_ID, s.SIT_ID]);
                }

                console.log(`[REPAIRED] ${orphan.name} (ID: ${orphan.MENU_ID}) restored from Donor ${donorId}`);
            }
            else 
            {
                console.log(`[REPAIR SKIP] No donor found for ${orphan.name} (ID: ${orphan.MENU_ID})`);
            }
        }
    }
    catch (e) 
